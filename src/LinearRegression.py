...
class linearModel:
	...
	def passNone(self, xidx, fidx, checkValid=True):
		...
		return (xidx, fidx) + self.feature.select(xidx, fidx, True, checkValid=checkValid)
	...
	def fitFeatureNormalize(self, Xindex, Findex, X, Y):
	...
	def passFeatureNormalize(self, Xindex, Findex, **kwargs):
		Xindex, Findex, X, Y = self.passNone(Xindex, Findex, **kwargs)
		...
		for i, ((FII, FI), mod) in \
			...
			res.append(sX)
			resFI += FI
		return Xindex, resFI, np.concatenate(res, 1), Y
	...
	def fitSelectByCor(self, Xindex, Findex, X, Y):
	...
	def passSelectByCor(self, Xindex, Findex, **kwargs):
		Xindex, Findex, X, Y = self.passFeatureNormalize(Xindex, Findex, **kwargs)
		...
		for i, j in enumerate(Findex):
			if j in c:
				resFI.append(j)
				res.append(X[:,i])
		return Xindex, resFI, np.stack(res, 1), Y
	...
	def fitSelectByL1(self, Xindex, Findex, X, Y):
	...
	def passSelectByL1(self, Xindex, Findex, **kwargs):
		Xindex, Findex, X, Y = self.passSelectByCor(Xindex, Findex, **kwargs)
	...
	def fitPCA(self, Xindex, Findex, X, Y):
	...
	def passPCA(self, Xindex, Findex = None, **kwargs):
		...
		for xidx in self.divide(Xindex):
			Xi, _, X, Y = self.passSelectByCor(xidx, Findex, **kwargs)
			resX.append(self.PCA.transform(X))
			resY.append(Y)
		return Xindex, None, np.concatenate(resX), np.concatenate(resY)
